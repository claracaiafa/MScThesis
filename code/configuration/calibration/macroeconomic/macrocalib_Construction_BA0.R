...
sectors_drop <- c("BR_RE", "BR_GS", "BR_GH2")
IO <- IO[!IO$icioiRow %in% sectors_drop, !colnames(IO)%in% sectors_drop]


X_baseline <- colSums(IO[,2:(n+1 - length(sectors_drop))])
VA_baseline <- colSums(IO[IO$icioiRowAgg %in% VASectors, 2:(n+1 - length(sectors_drop))])
VAc_baseline <- VA_baseline/X_baseline

Z_baseline <- IO %>% filter(icioiRowAgg %in% ProdSectors)
Z_baseline <- Z_baseline %>% select(2:(n+1 - length(sectors_drop)))
...
colnames(A_baseline) = colnames(IO[2:(n+1 - length(sectors_drop))])
rownames(A_baseline) = IO$icioiRowAgg[1:(n - length(sectors_drop))]

I <- diag(ncol(A_baseline))
